...
class BusOrderExpandListAdapter(val context: Context, val list: MutableList<ExpandGroupEmp>) : BaseExpandableListAdapter() {
    ...
    override fun getGroupView(groupPosition: Int, isExpanded: Boolean, convertView: View?, parent: ViewGroup?): View {
        val inflate = LayoutInflater.from(context).inflate(R.layout.view_group_bus_item, parent, false)
        ...
        ("票价: ￥" + list[groupPosition].price ).also { inflate.textView35.text = it }
        (list[groupPosition].mileage).also { inflate.textView36.text = "订单编号：$it" }
        return inflate
    }
